...
static METADATA metadata[] = {{"pmemdata_dummy", 0x00000000},
                       ...
                       {"pmemdata_wlan_bt_le", 0x00000000} 
};

static METADATA metadata_lpbk[]    = {{"pmemdata_lpbk", 0x00000000}}; 
static METADATA metadata_burn[]    = {{"pmemdata_burn", 0x00000000}};
static METADATA metadata_sniffer[] = {{"pmemdata_sniffer", 0x00000000}};
...
int  bl_cmd(PONEBOX_ADAPTER adapter, uint8 cmd, uint8 exp_resp, char *str)
{
	...
	if(bl_write_cmd(adapter, cmd, exp_resp, &regout_val) < 0) {
		ONEBOX_DEBUG( ONEBOX_ZONE_ERROR, ("%s:%d %s Command %0x writing failed..\n",__func__,
				__LINE__, str, cmd));
		goto fail;
	}
...
int bl_write_cmd(PONEBOX_ADAPTER adapter, uint8 cmd, uint8 exp_resp, uint16 *cmd_resp)
{
	...
	*cmd_resp = regout_val;
	...
	if(output == exp_resp) {
		ONEBOX_DEBUG( ONEBOX_ZONE_INFO, ("%s:%d Recvd Expected resp %x for cmd %0x\n",__func__,__LINE__, output, cmd));
	} else {
		ONEBOX_DEBUG( ONEBOX_ZONE_ERROR, ("%s:%d Recvd resp %x for cmd %0x\n",__func__,__LINE__, output, cmd));
		goto fail;
	}
	return ONEBOX_STATUS_SUCCESS;
...
#define ADDR_OFFSET 16
	...
	bl_hdr.image_no = adapter->coex_mode; /*Image number */
	bl_hdr.check_sum = ONEBOX_CPU_TO_LE32(*(uint32 *)&flash_content[CHECK_SUM_OFFSET]);
	bl_hdr.flash_start_address = ONEBOX_CPU_TO_LE32(*(uint32 *)&flash_content[ADDR_OFFSET]);
	bl_hdr.flash_len = ONEBOX_CPU_TO_LE32(*(uint32 *)&flash_content[LEN_OFFSET]);
...
CHECK_CRC_CMD:
	...
	if(bl_write_cmd(adapter, CHECK_CRC, CMD_PASS, &regout_val) < 0) {
		...
		if((regout_val & 0xff) == CMD_FAIL) {
			ONEBOX_DEBUG( ONEBOX_ZONE_ERROR, ("CRC Fail.. Proceeding to Upgrade mode\n"));
			goto FW_UPGRADE;
		}
	}
	bl_cmd_stop_timer(adapter);
