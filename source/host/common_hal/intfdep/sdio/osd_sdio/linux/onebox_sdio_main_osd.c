...
#elif KERNEL_VERSION_GREATER_THAN_2_6_(26)
static const struct sdio_device_id onebox_IdTable[] = 
{
...
void ulp_read_write(PONEBOX_ADAPTER adapter, uint16 addr, uint16 *data, uint16 len_in_bits)
{
	...
	sdio_master_reg_write(adapter, GSPI_DATA_REG0, *(uint16 *)&data[0], 2);
...
void reset_chip(PONEBOX_ADAPTER adapter)
{
...
#if 1
	ONEBOX_DEBUG(ONEBOX_ZONE_INFO, ("Writing disable to wakeup register in %s line %d\n", __func__, __LINE__));
	...
	{
		ONEBOX_DEBUG(ONEBOX_ZONE_ERROR,
				(TEXT("%s: Failed to Write SDIO WAKEUP REG\n"),__func__));
		return;
	}
	msleep(3);
	...
	ONEBOX_DEBUG(ONEBOX_ZONE_INFO,
			(TEXT("%s: Intr Status Register value = %d \n"),__func__, sdio_interrupt_status));
#endif
