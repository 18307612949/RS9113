...
{      
	...
	if (argc == 12 )
	{
		...
		per_params.ctry_region = atoi(argv[11]);
#if 1
		...
		{
				if(!islegacyrate(per_params.rate)) {
						printf("Invalid Rate in 11J mode\n");
						return -1;
				}
	...
	{
		printf("\nUSAGE to start transmit: %s tx_power rate length tx_mode channel ExtPA-Enable Rate_flags Aggr_enable no_of_packets delay ctry_region\n",argv[0]);
