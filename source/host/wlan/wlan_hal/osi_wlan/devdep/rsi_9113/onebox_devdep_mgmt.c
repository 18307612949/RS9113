 ...
 *  on failure
 */
static ONEBOX_STATUS onebox_bb_buffer_request(WLAN_ADAPTER w_adapter,
                                             uint16 *bb_buf_vals,
                                             uint16 num_of_vals)
{
	onebox_mac_frame_t *mgmt_frame;
...
ONEBOX_STATUS onebox_ant_sel(WLAN_ADAPTER w_adapter, uint8 value)
{
	onebox_mac_frame_t *mgmt_frame;
	ONEBOX_STATUS status_l;
	uint8  pkt_buffer[MAX_MGMT_PKT_SIZE];
  ...
  ONEBOX_DEBUG(ONEBOX_ZONE_INFO,("ONEBOX_IOCTL: ANT_SEL REQUEST\n"));
...
ONEBOX_STATUS onebox_load_radio_caps(WLAN_ADAPTER w_adapter)
{
  ...
  mgmt_frame->desc_word[7] |= ONEBOX_CPU_TO_LE16(ONEBOX_LMAC_CLOCK_FREQ_80MHZ);	// Radio config info	
	...
	radio_id = 0;
	mgmt_frame->u.radio_caps.sifs_tx_11n = w_adapter->sifs_tx_11n;
	mgmt_frame->u.radio_caps.sifs_tx_11b = w_adapter->sifs_tx_11b;
	mgmt_frame->u.radio_caps.slot_rx_11n = w_adapter->slot_rx_11n;
 	mgmt_frame->u.radio_caps.ofdm_ack_tout = w_adapter->ofdm_ack_tout;
  	mgmt_frame->u.radio_caps.cck_ack_tout = w_adapter->cck_ack_tout;
 	mgmt_frame->u.radio_caps.preamble_type = w_adapter->preamble_type;
	...
	if(wme_sta.cap_wmeParams[0].wmep_aifsn >= 2)
	{
	...
	if(wme.cap_wmeParams[0].wmep_aifsn >= 2)
	{
		mgmt_frame->u.radio_caps.qos_params[5].aifsn_val_q = (wme.cap_wmeParams[0].wmep_aifsn - 2);
	}
	else
	{
	...
	for(ii = 2; ii < (MAX_HW_QUEUES - 8); ii++)
	{
		...
		mgmt_frame->u.radio_caps.qos_params[ii + 4].aifsn_val_q = (wme.cap_wmeParams[ii].wmep_aifsn);
              ...
              if (msg_type == TA_CONFIRM_TYPE && sub_type == BOOTUP_PARAMS_REQUEST)
							{
								...
								if (d_assets->band_version == 0x03) {
									ONEBOX_DEBUG(ONEBOX_ZONE_ERROR, (TEXT("!!!! Dual band supported!!!!!\n")));
									w_adapter->operating_band = BAND_2_4GHZ;
									w_adapter->band_supported = 1;
								...
								w_adapter->eeprom.length = (IEEE80211_ADDR_LEN + WLAN_MAC_MAGIC_WORD_LEN
										+ WLAN_HOST_MODE_LEN);
								w_adapter->eeprom.offset = WLAN_MAC_EEPROM_ADDR;
								ONEBOX_DEBUG(ONEBOX_ZONE_MGMT_RCV,
										(TEXT("%s: Read MAC Addr %d, len %d\n"), __func__,  w_adapter->eeprom.offset, w_adapter->eeprom.length));
								if(eeprom_read(w_adapter) == ONEBOX_STATUS_SUCCESS)
								{
									w_adapter->fsm_state = FSM_EEPROM_READ_MAC_ADDR;
								}
								else
								{
									ONEBOX_DEBUG(ONEBOX_ZONE_ERROR,
				...
				switch (sub_type)
				{
							...
							TAILQ_FOREACH(vap, &ic->ic_vaps, iv_next)
							{
								...
								else if (vap->iv_opmode == IEEE80211_M_STA)
								{
									if ( (ic->bgscan_host_triggered && 
										...
										printk("Indicating Scan complete event =======>\n");
					...
					{	
						...
						TAILQ_FOREACH(vap, &ic->ic_vaps, iv_next)
						{
							if(vap == NULL)
								break;
							else if ((vap->iv_opmode == IEEE80211_M_STA) && (ic->ic_flags & IEEE80211_F_SCAN))
							{
...
#define P2P_GRP_GROUP_OWNER  0x0
  ...
  uint8 mpdu_density_map[] = {0,1,1,1,2,4,8,16};
	...
	switch(notify_event)
	{
      ...
      mgmt_frame->u.peer_notify.mpdu_density = mpdu_density_map[((ni->ni_htparam >> 2) & 0x7)];
      ONEBOX_DEBUG(ONEBOX_ZONE_INFO, (TEXT("mpdu Density %d\n"), mgmt_frame->u.peer_notify.mpdu_density));
			break;
			...
			&& (vap->iv_flags & IEEE80211_F_PRIVACY))
  {
    ...
    status = hal_load_key( w_adapter, w_adapter->wep_key[vap_id][0][0], 
...
ONEBOX_STATUS onebox_send_internal_mgmt_frame(WLAN_ADAPTER w_adapter, uint16 *addr, uint16 len)
{
	...
	w_adapter->os_intf_ops->onebox_memset(netbuf_cb->data,0,(FRAME_DESC_SZ + extnd_size ));
	...
	length_in_desc = (mgmt_frame->desc_word[0] & 0xfff); 
	mgmt_frame->desc_word[0] = ONEBOX_CPU_TO_LE16((length_in_desc + extnd_size)	|
...
//	mgmt_frame->desc_word[4] = ONEBOX_CPU_TO_LE16((key_descriptor) | (1<<5) | (1<<4));
	...
	if(data) {
	w_adapter->os_intf_ops->onebox_memcpy(&mgmt_frame->u.set_key.key, data, 4*32);
...
ONEBOX_STATUS onebox_send_reset_mac(WLAN_ADAPTER w_adapter)
{
	...
	if (d_assets->antenna_diversity)
		mgmt_frame->desc_word[6] = d_assets->antenna_diversity;
...
ONEBOX_STATUS set_bb_rf_values (WLAN_ADAPTER w_adapter, struct iwreq *wrq )
{
	...
	else if(type == BUF_READ_REQ || type == BUF_WRITE_REQ )
	{
		...
		if(onebox_bb_buffer_request(w_adapter, w_adapter->bb_rf_params.Data, w_adapter->bb_rf_params.no_of_values) != ONEBOX_STATUS_SUCCESS)
		{
			return ONEBOX_STATUS_FAILURE;
		}
	}
