 ...
 */
static ONEBOX_STATUS prepare_onair_data_pkt_desc(WLAN_ADAPTER w_adapter, 
                                  netbuf_ctrl_block_t *netbuf_cb,
                                  int8 q_num)
{
		uint16 *frame_desc;
...
static ONEBOX_STATUS prepare_onair_mgmt_pkt_desc(WLAN_ADAPTER w_adapter, 
                                  netbuf_ctrl_block_t *netbuf_cb)
{
	uint16 *msg = NULL;
	int msg_len = 0;
...
//	netbuf_cb->retry_count = 0;
	...
	if ((*(uint16 *)&netbuf_cb->data[12]) == htons(ETHERTYPE_IPV6)) {
		tos = (uint8_t)(ntohl((*(uint32 *)&netbuf_cb->data[14])) >> 20);
		tos >>= 5;
	} else {
