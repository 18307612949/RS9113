 ...
 * @section DESCRIPTION
 *
 * This file contians freebsd and driver interface functions.
 */
...
void core_net80211_attach(WLAN_ADAPTER w_adapter)
{
	...
	ic->coex_mode = d_assets->coex_mode;
...
void onebox_scan_start(struct ieee80211com *ic)
{
	...
	if (w_adapter->scan_count > MAX_SCAN_PER_ANTENNA) {
		...
		w_adapter->devdep_ops->onebox_program_ant_sel(w_adapter, antenna_value);
                                       ...
                                       uint8_t macaddr[IEEE80211_ADDR_LEN])
{
	...
	if (p2p_enable)
		wdev = w_adapter->net80211_ops->onebox_cfg80211_register_wireless_dev(vap->iv_ifp, ic->cfg_priv, 9);
	else
...
void onebox_vap_delete(struct ieee80211vap *vap, uint8 wait_for_lock)
{
	...
	if((w_adapter->sc_nstavaps) && (w_adapter->sc_nvaps < 2)) {
		TAILQ_FOREACH(vap_temp, &ic->ic_vaps, iv_next)
		{
			if(vap_temp->iv_opmode == IEEE80211_M_STA) {
				if(vap_temp->hal_priv_vap->ps_params.ps_en){
					update_pwr_save_status(vap_temp, PS_ENABLE, VAP_DELETE_PATH);
				}
			}
		}
	}
...
int core_send_station_info(struct ieee80211_node *ni, int notify_event)
{
	...
	if(notify_event == STA_CONNECTED)
	{
		...
		else if(notify_event == STA_ADDBA_DONE)
		{
			...
			if(ni->ni_tx_ampdu[ac].txa_flags & IEEE80211_AGGR_RUNNING)
			{
                      ...
                      uint32 cipher)
{
	...
	if(cipher == IEEE80211_CIPHER_WEP)
	{
	  ...
	  w_adapter->wep_keylen[vap_id][k->wk_keyix] = key_len;
		w_adapter->os_intf_ops->onebox_memcpy(&w_adapter->wep_key[vap_id][0][k->wk_keyix][0],key_buf,key_len);
...
#ifdef RSI_CCX
		if(ni_sta->ni_vap->hal_priv_vap->cckm_resp_sent) {
...
//	uint16 mcs_1[] = {13, 26, 39, 52, 78, 104, 117, 130};
	...
	w_adapter->auto_rate_params.failure_limit      = 3; 
	w_adapter->auto_rate_params.Initial_boundary   = 3;
	w_adapter->auto_rate_params.max_threshold_limt = 27;

	mgmt_frame->u.auto_rate.aarf_rssi = (((uint16)3 << 6) | (uint16)(18 & 0x3f));;
	mgmt_frame->u.auto_rate.moderate_rate_inx =	(((rs.rs_nrates + htrs.rs_nrates)/2) - 1) ; 
	mgmt_frame->u.auto_rate.collision_tolerance = 3;
  ...
  if (htrs.rs_nrates) {
	...
	else	/* sorting through the rates to get the min_rate*/
	{
		...
		for (ii = 0; ii < MAX_BG_SUPP_RATES; ii++) {
			if (jj == w_adapter->std_rates[ii]) {
				min_rate = w_adapter->rps_rates[ii]; 
				break;	
			}
		}
	}
	...
	if(htrs.rs_nrates && rs.rs_nrates)/** BG/N Mode **/
	{
		...
		for( kk = 0; kk < (rs.rs_nrates + htrs.rs_nrates); kk++)
		{
			...
			if(rs.rs_rates[ii] < mcs[jj])
			{
				...
				mgmt_frame->u.auto_rate.supported_rates[kk] = ((uint16)mcs_rates[jj]);
				if(jj == 0)
				{
					kk++;
		...
		for(ii = 7; ii >= 0; ii--)
		{
				if (ht_rate_bitmap & BIT(ii)) {
						if (((ni->ni_htcap & IEEE80211_HTCAP_SHORTGI20) &&(ic->ic_curchan->ic_flags & IEEE80211_CHAN_HT20))  || 
										((ni->ni_htcap & IEEE80211_HTCAP_SHORTGI40) && (ic->ic_curchan->ic_flags & IEEE80211_CHAN_HT40))) {
		...
		for (ii = remaining; ii > 2; ii--, kk++) { //check this, in the first loop RSI_RATE_36 mght not be a supported rate
			...
			if (min_mcs_rate > RSI_RATE_MCS4) {
				if(rs.rs_nrates > 4) { /*11G support*/
					mgmt_frame->u.auto_rate.supported_rates[kk] = RSI_RATE_36;
				} else {
					mgmt_frame->u.auto_rate.supported_rates[kk] = RSI_RATE_1;
				}
			} else {
	...
	//jj += kk;
	mgmt_frame->u.auto_rate.num_supported_rates = (rs.rs_nrates + htrs.rs_nrates)*2;
	//ONEBOX_DEBUG(ONEBOX_ZONE_ERROR, (TEXT("Num of supported rates = %d\n",mgmt_frame->u.auto_rate.num_supported_rates)));
	mgmt_frame->desc_word[0] = ONEBOX_CPU_TO_LE16((sizeof(mgmt_frame->u.auto_rate) + mgmt_frame->u.auto_rate.num_supported_rates * 2) | 
														                      (ONEBOX_WIFI_MGMT_Q << 12));
	//mgmt_frame->u.auto_rate.moderate_rate_inx =(uint16)rate_idx; 

	ONEBOX_DEBUG(ONEBOX_ZONE_AUTORATE, (TEXT("Auto rate frame dump\n")));
	w_adapter->core_ops->onebox_dump(ONEBOX_ZONE_AUTORATE, (PUCHAR)mgmt_frame, (FRAME_DESC_SZ + sizeof(mgmt_frame->u.auto_rate) + (2*mgmt_frame->u.auto_rate.num_supported_rates)));
	status = w_adapter->devdep_ops->onebox_send_internal_mgmt_frame(w_adapter, (uint16 *)mgmt_frame,  (FRAME_DESC_SZ + (sizeof(mgmt_frame->u.auto_rate)+ 2 * mgmt_frame->u.auto_rate.num_supported_rates)));
