 ...
 * @section DESCRIPTION
 *
 * The file contains the linux specific interface related code
 */
...
int ether_ifattach(struct ifnet *ifp, const u_int8_t *lla)
{
	...
	{
		printk("Unable to register the device%s\n",ifp->name);
		return -1;
	}
	return 0;
}
...
void if_free(struct ifnet *ifp, uint8_t wait_for_lock)
{
	...
	IEEE80211_DBG_PRINT(vap, IEEE80211_MSG_DEBUG, ("rtnl_lock status is %d \n",rtnl_is_locked()));

	//ret = rtnl_trylock();
	//printk("rtnl_trylock status is %d\n", ret);
#if 1
