    ...
    case RSI_COMMON_REQ_OPERMODE:
      {
        ...
        if(( rsi_bytes4R_to_uint32(rsi_opermode->opermode ) & 0x00FF) == 2)
        {
          rsi_uint32_to_4bytes(rsi_opermode->tcp_ip_feature_bit_map, (TCP_IP_FEAT_DHCPV4_CLIENT | TCP_IP_FEAT_DNS_CLIENT ));
        }
        else
        {
...
if(rsi_driver_cb->common_cb->expected_response != RSI_WLAN_RSP_ASYNCHRONOUS)
{
