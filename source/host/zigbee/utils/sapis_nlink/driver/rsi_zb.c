...
int32_t rsi_zigb_send_cmd(uint8_t *cmd, rsi_pkt_t *pkt)
{
  ...
  switch(cmd[1])
    ...
    case ZIGBEESETPOLLRATE:
      {
	payload_size = sizeof(setPollFrameSnd);
      }
      break;
  ...
  host_desc[15] = cmd[1];
	...
	if(cmd[1] == RSI_COMMON_REQ_OPERMODE)
	{
        ...
        status = rsi_zigb_execute_cmd( cmd , ppl ,payload_size );
...
void rsi_zigb_app_cb_handler(uint8_t cmd_id, uint8_t *buffer)
{
  ...
  switch(cmd_id)
  {
    ...
    case ZIGBEEZDPAPPRESPONSE:
    {
	    ...
	    buf_id= buffMgmt_allocateBuffer ( RSI_ZIGB_LARGE_BUFFER_SIZE );
	    if(buf_id != 0xFF) {
		    buf_ptr = buffMgmt_getBufferPointer(buf_id);
		    memUtils_memCopy(buf_ptr,buffer,RSI_ZIGB_LARGE_BUFFER_SIZE);
		    zigb_specific_cb->zigb_zdp_app_response_handler(buf_id);     
	    }
    }
    break;
    default:
