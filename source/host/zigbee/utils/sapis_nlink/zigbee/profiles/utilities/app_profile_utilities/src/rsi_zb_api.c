...
/* Set Poll Rate */
const uint8_t   rsi_zigb_frameSetPollRate[RSI_ZIGB_BYTES_2] = {MANAGEMENT_INTERFACE, ZIGBEESETPOLLRATE};
...
uint8_t TH_setPollRate(uint16_t poll_rate)
{
  rsi_pkt_t *pkt;
  uint8_t status = RSI_SUCCESS;
  setPollFrameSnd *poll=NULL;
  ...
  poll = (setPollFrameSnd*)pkt->data;
  poll->poll_rate = poll_rate;

  //! send  command
   rsi_zigb_send_cmd((uint8_t *)rsi_zigb_frameSetPollRate, pkt);
