                                      ...
                                      App_Cluster_Def_t ** pZCLClusterDefinition )
{
    ...
    for ( epindex = 0; epindex < g_MAX_NO_OF_ENDPOINTS_c ; epindex++ ){
        ...
        if ( endpointId == pDeviceInfo->endpoint  ){
	      ...
	      if(pDeviceInfo->p_list_of_cluster_def_ptrs[clstrindex].p_cluster_info != g_NULL_c)
	      {
...
void App_Stack_Information ( uint8_t msg_type, uint8_t msg_index )
{
	...
	printf("Msg Type is %d\n",stack_info_conf->msg_type);
	printf("Attribute id is %d\n",stack_info_conf->attribute_id);
	...
	if(stack_is_up)
   	{
	    ...
	    if(stack_is_up)
	        currentTime = currentTick = stack_is_up = 0;
