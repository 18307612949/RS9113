...
#if(g_POWER_PROFILE_SERVER_CLUSTER_ENABLE_d == 1)
extern Power_Profile_Cluster_t power_profile_cluster_values;//remove this on test harness integration//added
#endif
/*******************************************************************************
* Public Functions
*******************************************************************************/
    ...
    uint8_t responseType,uint8_t profileId, bool serverToClient)
{
    ...
    uint8_t tempProfileId = 0xff,i;

    uint8_t payloadLength = 0x00,profile_Count = 0x00,energyPhaseCount = 0x00;

    uint8_t energyPhaseCountListIndex = 0x00;
    CountTracker_t energyPhaseCountList[g_MAXIMUM_NO_OF_PROFILES_c];

    for(i=0; i < g_MAXIMUM_NO_OF_PROFILES_c; i++)
	memUtils_memSet((uint8_t *)&energyPhaseCountList[i],0x00,2);
    ...
    if((profileId == g_ALL_ACTIVE_POWER_PROFILES_c) ||
        ...
        function*/
        for(uint8_t loopIndex = 0x00;loopIndex <= profile_Count ;loopIndex++){
		...
		pAppDataReq->dest_endpoint = 0x01;
