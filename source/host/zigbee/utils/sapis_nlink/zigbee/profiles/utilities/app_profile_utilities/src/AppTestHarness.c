...
Persistent_NIB_t *gp_Persistent_NIB;
extern Startup_Attribute_Set_t Startup_Attribute_Set_Default;
#define m_SIZE_CONFIGURATION_c				            0x80
uint32_t g_NVM_SAS_Start_Offset_c;
uint8_t temp_re;

#define m_MAC_EXTENDED_ADDRESS_ATTRIBUTE_LENGTH_c 0x08
...
void testHrns_passMessageDown ( uint8_t buffer_index )
{
    ...
    /*Check if the type of message ids for data and forward it to the data path*/
    		...
    		printf("Executing Test Harness Data Path[%X]\n",Test_Harness_Data_Path);
	...
	management path. */
    	...
    	printf("Executing Test Harness Management Path[%X]\n",Test_Harness_Management_Path);
...
//#endif
      ...
      printf("\n Asdu:");
      for(i=0;i< pApsdeData->asdulength;i++)
        printf(" 0x%x",pApsdeData->asdu[i] );
    ...
    case m_RESET_REQUEST_c:
                              ...
                              *(p_buffer_rx+1) = 0x00;
                              testHrns_passMessageUp ( msg_index, 2 );
			     			  free_buf = buffMgmt_getNumberofFreeBuffers(g_LARGE_BUFFER_c);
			      			  printf("\n No of free buffer available is %d\n",free_buf);
                              return;
...
void App_updatePriceEvents ( uint8_t* pPriceData )
{
	...
	memUtils_memSet( (uint8_t*)&pPriceTable.endTime, g_ZERO_c, 0x04 );
    pPriceData += 0x04;
