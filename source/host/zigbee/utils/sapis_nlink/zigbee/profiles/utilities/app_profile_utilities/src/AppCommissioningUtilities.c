                                    ...
                                    APSDE_Data_Indication_t *pBuffer )
{
    ...
    switch ( pZCLEvent->eventId )
    {
		...
		if(saveCmdParam.Index == 1)
		{
	                ZDOupdateSAS((uint8_t*)&localAttributeSet,saveCmdParam.Index);
	                //rsi_zigb_update_sas((uint8_t*)&localAttributeSet);
        	        memUtils_memCopy(&gp_Active_Startup_Attrib_Set,&localAttributeSet,sizeof(Startup_Attribute_Set_t));
		}
