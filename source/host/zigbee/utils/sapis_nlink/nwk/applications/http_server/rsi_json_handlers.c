...
uint8_t* rsi_json_object_stringify(uint8_t *json_object_string, rsi_json_object_t* json_object)
{
  ...
  sprintf((char *)json_object_string,
      "{\"ssid\":\"%s\",\"channel\":%d,\"secenable\":%s,\"sectype\":%d,\"psk\":\"%s\",\"cbox2\":%s,\"accy\":%d}",
      json_object->ssid,
      json_object->channel,
      (json_object->sec_enable == 1 ? "true" : "false"),
      json_object->sec_type,
      json_object->psk,
      (json_object->checkbox_2 == 1 ? "true" : "false"),
      json_object->accelerometer_y
      );
...
{
    rsi_strcpy(json_object->ssid, "redpine");
    json_object->channel          =  0;
    json_object->sec_enable       =  1;
    json_object->sec_type         =  1;
    rsi_strcpy(json_object->psk, "123456789");
    json_object->accelerometer_y  =  6;
    json_object->checkbox_2       =  0;
}
...
void rsi_json_object_data_update_helper(rsi_json_object_t* json_object, uint8_t* json)
{
    ...
    rsi_json_extract_int(json,(uint8_t *) "sec_type", &(json_object->sec_type));
